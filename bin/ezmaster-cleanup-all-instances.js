...
request.get(url, ezmaster.request_option, function (error, response, instances) {
  if (error) return console.error(error);
  instances = JSON.parse(instances);
  Object.keys(instances).forEach(function (instance) {
    const containerId = instances[instance].containerId;
    if (!containerId) return;
    request.delete(EZMASTER_BASEURL + '/-/v1/instances/' + containerId, ezmaster.request_option, function (error, response, deleteDone) {
